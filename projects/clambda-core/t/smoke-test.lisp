...
Returns T on success, signals error on failure."
  (format t "~%=== clambda-core smoke test ===~%~%")
  ...
  (let* ((client (cl-llm:make-client
         ...
         (registry (clambda:make-builtin-registry))
         ...
         (opts (clambda:make-loop-options
                :max-turns 5
                :verbose   t)))

    (format t "[test] Agent: ~a~%" (clambda:agent-name agent))
    (format t "[test] Tools: ~{~a~^, ~}~%"
            (clambda/tools:list-tools registry))
...
(defun test-tool-registry ()
  ...
  (let ((registry (clambda:make-tool-registry)))
    ...
    (clambda:define-tool registry "greet"
      "Greet someone by name."
      (("name" "string" "Person's name"))
      (format nil "Hello, ~a!" name))

    ;; Check it's registered
    (assert (clambda/tools:find-tool registry "greet"))
    ...
    (let* ((fake-tc (cl-llm/protocol:make-tool-definition
      ...
      (let* ((entry (clambda/tools:find-tool registry "greet"))
             (handler (clambda/tools::tool-entry-handler entry))
             (result (funcall handler ht)))
        (assert (clambda/tools:tool-result-ok result))
        (assert (string= (clambda/tools:tool-result-value result) "Hello, Lisp!"))
...
(defun test-builtin-exec ()
  ...
  (let* ((registry (clambda:make-builtin-registry))
         (exec-entry (clambda/tools:find-tool registry "exec")))
    ...
    (let* ((args (make-hash-table :test #'equal))
           ...
           (result (funcall (clambda/tools::tool-entry-handler exec-entry) args)))
      (declare (ignore _))

      (assert (clambda/tools:tool-result-ok result))
      (let ((output (clambda/tools:tool-result-value result)))
...
(defun test-session ()
  ...
  (let* ((agent (clambda:make-agent :name "test-agent"))
         (session (clambda:make-session :agent agent)))

    (assert (clambda:session-id session))
    (assert (= 0 (clambda:session-message-count session)))
    (format t "PASS: session created, id=~a~%" (clambda:session-id session))

    (clambda:session-add-message session (cl-llm:user-message "hello"))
    (assert (= 1 (clambda:session-message-count session)))
    (format t "PASS: message added~%")

    (clambda:session-clear-messages session)
    (assert (= 0 (clambda:session-message-count session)))
...
(defun run-all-tests ()
  "Run all clambda-core tests. Returns T if all pass."
  (let ((results nil))
